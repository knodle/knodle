...
class CrossWeighWeightsCalculator:
    ...
    def calculate_weights(self) -> np.ndarray:
        ...
        for partition in range(self.denoising_config.cw_partitions):
            ...
            logger.info(
                "============ CrossWeigh Partition {} is done ============".format(
                    partition + 1
                )
            )
        self._save_weights()
        ...
        logger.info(
            "======= Denoising with CrossWeigh is completed, sample_weights are saved to {} =======".format(
    ...
    def get_cw_data(
        ...
        train_samples, train_labels, train_idx = self._get_cw_samples_labels_idx(
            labels, train_rules_idx, test_idx
        )

        # debug: check that splitting was done correctly
        check_splitting(
            train_samples, train_labels, train_idx, self.inputs_x.tensors[0], labels
        )
        check_splitting(
            test_samples, test_labels, test_idx, self.inputs_x.tensors[0], labels
        )
    ...
    def _get_cw_samples_labels_idx(
        ...
        for idx in indices:
            ...
            if check_intersections is not None:
                matched_sample_ids = return_unique(
                    matched_sample_ids, check_intersections
                )
            cw_samples = torch.cat(
                (cw_samples, self.inputs_x.tensors[0][matched_sample_ids]), 0
            )
    ...
    def cw_train(self, train_loader: DataLoader):
        ...
        for _ in tqdm(range(self.denoising_config.cw_epochs)):
            for tokens, labels, _ in train_loader:
                self.denoising_config.criterion.weight = (
                    self.denoising_config.class_weights
                )
                self.denoising_config.criterion.reduction = "none"
                self.denoising_config.optimizer.zero_grad()
                output = self.model(tokens)
                loss = self._get_train_loss(
                    self.denoising_config.criterion, output, labels, self.sample_weights
                )
                ...
                if self.denoising_config.use_grad_clipping:
                    nn.utils.clip_grad_norm_(
                        self.model.parameters(), self.denoising_config.grad_clipping
                    )
    ...
    def _get_train_loss(self, criterion, output, labels, weights):
        ...
        ).sum() / self.denoising_config.class_weights[labels].sum()
            ...
            for tokens, labels, idx in test_loader:
                ...
                for curr_pred in range(len(predictions)):
                    ...
                    if guess not in gold_classes:
                        ...
                        wrong_predictions += 1
        logger.info(
            "Correct predictions: {:.3f}%, wrong predictions: {:.3f}%".format(
