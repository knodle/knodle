...
class SnorkelTrainer(MajorityVoteTrainer):
    ...
    def _snorkel_denoising(
        ...
        if self.trainer_config.caching_folder:
            cache_file = self.trainer_config.get_snorkel_cache_file()
            if os.path.isfile(cache_file):
                logger.info(f"Loaded snorkel input from cache: {cache_file}")
                model_input_x, label_probs = joblib.load(cache_file)
        ...
        if self.trainer_config.caching_folder:
            ...
            joblib.dump((model_input_x, label_probs), cache_file)
