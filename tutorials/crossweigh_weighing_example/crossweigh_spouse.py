...
def train_crossweigh(
    ...
    train_df, dev_df, test_df, rule_matches_z, test_rule_matches_z, mapping_rules_labels_t, y_dev, y_test = \
        read_train_dev_test(path_to_data)
    ...
    dev_labels = TensorDataset(Tensor(y_dev))
...
def read_train_dev_test(target_path: str):
    ...
    dev_df = load(os.path.join(target_path, 'df_test.lib'))
    ...
    y_dev = load(os.path.join(target_path, 'Y_dev.lib'))
    y_test = load(os.path.join(target_path, 'Y_test.lib'))

    return train_df, dev_df, test_df, train_rule_matches_z, test_rule_matches_z, mapping_rules_labels_t, y_dev, y_test
