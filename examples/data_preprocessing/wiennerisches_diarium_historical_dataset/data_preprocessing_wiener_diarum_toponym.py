...
sterbelisten_strip_html1 = re.sub(r"<hr/>|<p>|</p>|#+|=| =|<html>|</html>|\r|\b \.|\b# \.|(|)", "",
                                  sterbelisten_strip_html_1)  # <mark>|</mark>
...
print(f"Number of </mark> tags: {len(re.findall('</mark>', sterbelisten_html))}")
print(f"Number of unclosed tags: "
      f"{len(re.findall('<mark>', sterbelisten_html)) - len(re.findall('</mark>', sterbelisten_html))}")
...
# we collect our tagged sentences in samples, these contain None for each word that is no place name
# and the corresponding ID to each word that is part of a place name
samples = []
